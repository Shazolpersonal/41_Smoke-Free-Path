...
export default function MilestoneDetector() {
  ...
  const prevCompletedCountRef = useRef<number>(state.planState.completedSteps.length);
  ...
  useEffect(() => {
    ...
    const milestone = detectMilestone(state.planState.completedSteps, state.milestones);
  ...
  }, [state.planState.completedSteps, state.milestones, state.planState.isActive, hydrated, dispatch, router]);
