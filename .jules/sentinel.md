...
## 2026-05-18 - Missing Input Length Limits (DoS/Memory Exhaustion)
**Vulnerability:** Several `TextInput` components across the application (e.g., in forms, floating labels, search fields, and trigger logs) did not have `maxLength` properties defined.
**Learning:** React Native `TextInput` without a `maxLength` property allows users or automated scripts to paste arbitrarily large strings (e.g., megabytes of text) into the input field. This can lead to severe UI lag, memory exhaustion, and application crashes (Denial of Service). This is a known risk when input validation defaults are missing.
**Prevention:** Always enforce a `maxLength` property on all `TextInput` components. For generic/reusable UI input components like `FloatingLabelInput` or `FormInput`, accept `maxLength` as a prop and provide a generous fallback (e.g., `maxLength={props.maxLength || 1000}`) to prevent unbounded input while not arbitrarily truncating valid long inputs.
