...
## 2026-07-07 - Input Length Limits (DoS Mitigation)
**Vulnerability:** Multiple text input fields across the application (such as trigger logs, search bars, and profile forms) lacked `maxLength` constraints. This missing input validation allowed for the possibility of extremely large strings being pasted or typed into the application, leading to potential memory exhaustion or client-side Denial of Service (DoS) crashes.
**Learning:** Relying solely on the UI for input constraint is insecure. Without explicit limits set at the `TextInput` level, the React Native runtime can attempt to process unbound string sizes, consuming excessive memory and locking the JS thread.
**Prevention:** Always enforce strict and sensible `maxLength` properties on all user-facing `TextInput` components. For generic wrappers like `FloatingLabelInput` or `FormInput`, accept `maxLength` as a prop but provide a safe fallback (e.g., 1000 characters) to ensure no input is ever completely unbounded.
