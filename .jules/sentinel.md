...
**Vulnerability:** Several text input fields in the application lack explicit `maxLength` limits, which can be exploited by an attacker to cause Denial of Service (DoS) through memory exhaustion or by submitting excessively long payloads that could impact backend processing (if implemented).
**Learning:** React Native `TextInput` elements, if unrestricted, will allow arbitrary lengths of text. When combined with state management causing re-renders or when passed down to shared generic UI components (`FormElements`, `FloatingLabelInput`), this poses a significant resource exhaustion risk, particularly on mobile devices. Setting a generous default in shared components prevents widespread vulnerability without disrupting normal use.
