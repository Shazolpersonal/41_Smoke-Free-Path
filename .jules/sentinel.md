...
**Vulnerability:** Several `TextInput` components across the application (trigger logs, search inputs, custom form elements) did not have `maxLength` limits defined.
**Learning:** Unbound text inputs can be exploited by users pasting massive amounts of text. Because React Native syncs this text with the native layer and stores it in memory (and potentially large state updates), it can lead to memory exhaustion and effectively cause a Denial of Service (DoS) by crashing the application.
**Prevention:** Always enforce a strict `maxLength` attribute on all `TextInput` components (including custom wrapper components like `FloatingLabelInput` and `FormElements`) to mitigate memory exhaustion and DoS risks from unbounded user input.
