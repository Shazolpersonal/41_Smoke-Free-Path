...
## 2026-06-18 - Input Validation Missing (DoS Risk)
**Vulnerability:** Found common React Native text input components (`FormInput` and `FloatingLabelInput`) lacking default `maxLength` properties.
**Learning:** Without explicit length limits, a user or automated script could paste massive amounts of text (e.g., megabytes) into unconstrained inputs. In React Native, this string is held in JS memory and passed across the bridge to the native UI, which can lead to layout thrashing, extreme memory spikes, and application crashes (Client-side Denial of Service).
**Prevention:** Always enforce a generous but strict `maxLength` (e.g., 1000) on base `TextInput` wrapper components to prevent excessively large payloads from exhausting memory or disrupting UI rendering.
