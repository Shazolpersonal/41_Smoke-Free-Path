...
## 2024-06-03 - Denial of Service via Unbounded TextInput
**Vulnerability:** Found `TextInput` components across the application (such as trigger log note, library search, dua search, and custom form inputs) lacking `maxLength` constraints.
**Learning:** React Native's `TextInput` without a length limit allows users (or malicious automation) to paste arbitrarily large strings (e.g. megabytes of text) into the input. This can cause the JavaScript thread to freeze, resulting in memory exhaustion and Denial of Service (DoS), especially when this state is synchronized or stored.
**Prevention:** To mitigate Denial of Service (DoS) and memory exhaustion risks, always enforce input validation by explicitly setting `maxLength` attributes on all `TextInput` components (e.g., in forms, custom input wrappers like `FloatingLabelInput`, and log entry fields) throughout the application.
