...
## 2026-05-23 - Prevent DoS via Input Length Limits
**Vulnerability:** Missing maxLength on TextInputs (DoS/memory exhaustion risk)
**Learning:** Unrestricted text inputs can lead to memory exhaustion and application crashes if a user or script pastes excessively large strings.
**Prevention:** Always enforce input validation by explicitly setting `maxLength` on all `TextInput` components and wrappers.
