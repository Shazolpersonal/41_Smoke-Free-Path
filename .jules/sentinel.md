...
## 2026-05-20 - Missing Input Length Limits
**Vulnerability:** TextInput components missing maxLength limits, potentially allowing a user or script to paste massive amounts of text and cause a Denial of Service (DoS) through memory exhaustion.
**Learning:** React Native's TextInput handles layout calculations synchronously. Supplying an enormous string block can cause layout thrashing and out-of-memory errors on lower-end mobile devices. Even if an input only seems like a small note or search bar, missing maxLength represents an edge-case DoS risk.
**Prevention:** Always enforce a generic generous `maxLength` prop for custom wrappers (e.g. `maxLength={props.maxLength || 1000}`) and explicit smaller values for specific inputs like search bars to prevent layout engine crashes.
