...
## 2024-05-24 - Missing Input Validation / Length Limits (DoS risk)
**Vulnerability:** Found multiple `TextInput` components across the application (trigger logs, search inputs, reusable form elements) missing `maxLength` limits.
**Learning:** React Native's `TextInput` without a `maxLength` allows users (or malicious actors pasting data) to input exceptionally long strings. This can lead to severe performance degradation (UI freezing), memory exhaustion, and potential Denial of Service (DoS) when that data is processed or stored in memory/AsyncStorage. Reusable components must accept and utilize a generous fallback for this prop.
**Prevention:** Always enforce input validation by explicitly setting `maxLength` attributes on all `TextInput` components. For generic/reusable UI input components (like `FloatingLabelInput` or `FormInput`), accept `maxLength` as a prop and use a generous fallback (e.g., `props.maxLength || 1000`) rather than hardcoding small limits, to prevent unintended truncation of valid long inputs like URLs or API keys.
