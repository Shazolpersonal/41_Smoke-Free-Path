...
## 2024-05-22 - Input Length Limits (DoS Mitigation)
**Vulnerability:** Several core user inputs (including onboarding profile, trigger logs, and search bars) lacked explicit length restrictions, which could lead to excessively large payloads and possible application resource exhaustion (DoS) when processed.
**Learning:** React Native `TextInput` elements, if unrestricted, can capture large buffers. While the backend or storage layer might catch some length limits, the UI layer should proactively drop excessively large strings for defense-in-depth and smoother client performance.
**Prevention:** Always set a sensible `maxLength` (e.g., 100, 255, or 500 characters) on `TextInput` wrappers (like `FloatingLabelInput` and `FormInput`) and standalone search/note inputs.
