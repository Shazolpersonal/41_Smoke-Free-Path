...
## 2026-06-13 - Input Validation Enforcement
**Vulnerability:** Missing input length limits on user-facing text fields (DoS risk, UI freeze risk).
**Learning:** React Native `TextInput` components without `maxLength` constraints allow arbitrary-length pastes, which can freeze the UI thread or cause unexpected state management performance issues on mobile devices.
**Prevention:** Always add a reasonable `maxLength` prop to `TextInput` and any custom wrapper components like `FloatingLabelInput` and `FormInput` to enforce defense in depth.
