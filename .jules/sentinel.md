...
## 2026-05-18 - Missing Input Validation on TextInputs
**Vulnerability:** Missing input length limits on TextInputs (DoS risk)
**Learning:** React Native TextInputs without maxLength can cause memory exhaustion/DoS if users paste extremely large strings.
**Prevention:** Always enforce maxLength on TextInputs. For reusable components, accept it as a prop with a generous fallback (e.g., 1000) to prevent truncation of valid long inputs.
