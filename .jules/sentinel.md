...
## 2026-05-26 - Prevent Denial of Service and Memory Exhaustion via TextInput Inputs
**Vulnerability:** Unbounded TextInput components could allow users to paste or enter massive amounts of text, potentially causing the app to crash due to memory exhaustion or leading to uncontrolled state growth.
**Learning:** To mitigate DoS and memory exhaustion risks, always enforce input validation by explicitly setting `maxLength` attributes on all `TextInput` components (e.g., in forms, custom input wrappers like `FloatingLabelInput`, and log entry fields) throughout the application. This ensures limits are enforced at the UI layer.
**Prevention:** Review all `TextInput` instances to ensure they include an appropriate `maxLength` prop.
