...
## 2026-05-24 - Missing Input Length Limits (DoS risk)
**Vulnerability:** Found multiple `TextInput` components across the application (like `FloatingLabelInput`, `FormInput`, search bars, and note text areas) lacking explicit `maxLength` restrictions.
**Learning:** Without strict length boundaries, a user could paste extremely large strings into input fields. Since React Native processes these in memory on both the native and JS thread, very large inputs can cause frame drops, layout thrashing, excessive re-renders, and memory exhaustion leading to application crashes (a local Denial of Service).
**Prevention:** Always explicitly define `maxLength` on `TextInput` components. For generic wrappers like `FloatingLabelInput`, accept a `maxLength` prop and fall back to a generous default (e.g., `maxLength || 1000`) rather than leaving it unbounded.
