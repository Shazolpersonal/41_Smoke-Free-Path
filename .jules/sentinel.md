...
## 2026-05-17 - DoS via Unbounded Text Inputs
**Vulnerability:** Found multiple `TextInput` components (e.g., search inputs, note taking forms, generic floating labels) lacking `maxLength` attributes, making the app vulnerable to memory exhaustion and application freezing (Denial of Service) if users paste massively long strings.
**Learning:** React Native's `TextInput` without bounded lengths can consume unbounded memory when processing very large strings, leading to UI thread lockups and application crashes. Generic components like `FormInput` and `FloatingLabelInput` must propagate `maxLength` and provide safe defaults.
**Prevention:** To mitigate Denial of Service (DoS) and memory exhaustion risks, always enforce input validation by explicitly setting `maxLength` attributes on all `TextInput` components. For generic/reusable UI input components, accept `maxLength` as a prop and use a generous fallback (e.g., `props.maxLength || 1000`).
