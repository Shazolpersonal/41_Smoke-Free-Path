...
export default function DuaScreen() {
  ...
  const allDuas = useMemo(() => getDuasByCategory(activeCategory), [activeCategory]);
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.colors.primary }]}>
        ...
        <FlatList
          ...
          renderItem={({ item: cat }) => (
            <TouchableOpacity
              style={[
                styles.tab,
                { backgroundColor: activeCategory === cat.key ? theme.colors.surface : theme.colors.onPrimary + '33', paddingHorizontal: 14, paddingVertical: 7 },
              ]}
              onPress={() => {
            ...
            >
              <Typography
                ...
                style={[
                  styles.tabText,
                  { color: activeCategory === cat.key ? theme.colors.primary : theme.colors.onPrimary + 'DD', fontWeight: '600' },
                ]}
              >
                {cat.label}
              </Typography>
      ...
      <View style={[styles.searchWrapper, { backgroundColor: theme.colors.primary, paddingHorizontal: theme.spacing.md, paddingBottom: theme.spacing.md }]}>
        <TextInput
          style={[
            styles.searchInput,
            { backgroundColor: theme.colors.surface, color: theme.colors.text, paddingHorizontal: theme.spacing.md, paddingVertical: 9 },
          ]}
        ...
        ListEmptyComponent={
          <View style={[styles.emptyCard, { backgroundColor: theme.colors.surface, padding: theme.spacing.lg, marginTop: theme.spacing.sm }]}>
            <Typography variant="body" style={[{ color: theme.colors.textSecondary }]}>
              {searchQuery.trim() ? 'কোনো ফলাফল পাওয়া যায়নি' : 'এই বিভাগে কোনো দোয়া নেই।'}
            </Typography>
          </View>
        }
        renderItem={({ item }) => (
          <AnimatedDuaItem item={item} onPress={() => setSelectedDua(item)} categoryLabel={activeCategoryLabel} />
        )}
      />
      ...
      >
        {selectedDua && (
          <SafeAreaView style={[styles.modalSafe, { backgroundColor: theme.colors.surface }]}>
            <View style={[styles.modalHeader, { paddingHorizontal: theme.spacing.lg, paddingTop: theme.spacing.md, paddingBottom: theme.spacing.sm }]}>
              <Pressable
                onPress={() => setSelectedDua(null)}
                style={[styles.closeBtn, { backgroundColor: theme.colors.border, paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20 }]}
                accessibilityLabel="বন্ধ করুন"
                accessibilityRole="button"
              >
                <Typography variant="body" style={[styles.closeBtnText, { color: theme.colors.text, fontWeight: '600' }]}>✕ বন্ধ করুন</Typography>
              </Pressable>
            </View>
            <FlatList
              style={styles.modalScroll}
              contentContainerStyle={[styles.modalContent, { padding: theme.spacing.lg, paddingBottom: 48, alignItems: 'center' }]}
              showsVerticalScrollIndicator={false}
              data={[selectedDua]}
              keyExtractor={(item) => item.id}
              renderItem={({ item }) => (
                <>
                  <ArabicText text={item.arabicText} fontSize={32} style={[styles.modalArabic, { marginBottom: theme.spacing.md }]} />
                  <Typography variant="subheading" style={[styles.modalTranslit, { color: theme.colors.textSecondary, fontStyle: 'italic', marginBottom: theme.spacing.lg, lineHeight: 24 }]}>
                    {item.banglaTransliteration}
                  </Typography>
                  <View style={[styles.modalDivider, { backgroundColor: theme.colors.border, width: '60%', height: 1, marginBottom: theme.spacing.lg }]} />
                  <Typography variant="title" style={[styles.modalTranslation, { color: theme.colors.text, lineHeight: 28, marginBottom: theme.spacing.lg }]}>
                    {item.banglaTranslation}
                  </Typography>
                  <Typography variant="small" style={[styles.modalSource, { color: theme.colors.textDisabled }]}>
                    {item.source}
                  </Typography>
                </>
              )}
