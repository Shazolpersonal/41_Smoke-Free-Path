...
export function isStepAccessible(step: number, planState: PlanState): boolean {
  ...
  if (planState.activatedAt && new Date(planState.activatedAt).getTime() > Date.now()) {
    return false;
  }
...
export function getWeeklyTriggerSummary(
  ...
  let maxCount = 0;
  for (const [type, count] of Object.entries(counts) as [TriggerType, number][]) {
    if (count > maxCount) {
      maxCount = count;
      topTrigger = type;
    }
...
export function checkTriggerThreshold(
  ...
  const count = logs.filter(
    (log) =>
      log.type === type && new Date(log.timestamp).getTime() >= cutoff,
  ).length;
  return count > 3;
}
