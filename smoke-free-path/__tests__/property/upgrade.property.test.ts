...
const safeNullableIsoArb = fc.oneof(fc.constant(null), safeIsoArb);
...
const stepProgressArb: fc.Arbitrary<Record<number, StepProgress>> =
  fc.dictionary(
...
function buildAppStateArb(now: number, daysThreshold: number) {
  ...
  const oldTs = new Date(
    Math.max(MIN_TS, cutoffMs - 7 * 24 * 60 * 60 * 1000),
  ).toISOString();
  // Timestamps clearly after cutoff (recent) — 1 day after cutoff, capped at now
  const recentTs = new Date(
    Math.min(now, cutoffMs + 24 * 60 * 60 * 1000),
  ).toISOString();

  const oldLogsArb = fc.array(triggerLogWithTimestampArb(oldTs), {
    maxLength: 5,
  });
  const recentLogsArb = fc.array(triggerLogWithTimestampArb(recentTs), {
    maxLength: 5,
  });
  const oldSessionsArb = fc.array(cravingSessionWithStartTimeArb(oldTs), {
    maxLength: 5,
  });
  const recentSessionsArb = fc.array(cravingSessionWithStartTimeArb(recentTs), {
    maxLength: 5,
  });
...
describe("Property 19: Data Cleanup Preserves Critical Data", () => {
  ...
  it("milestones are preserved unchanged after cleanup", () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 90 }), (daysThreshold) => {
      ...
      fc.property(fc.integer({ min: 1, max: 90 }), (daysThreshold) => {
        ...
        fc.assert(
          fc.property(stateArb, (state) => {
            ...
            expect(result.cravingSessions).toHaveLength(
              recentInputSessions.length,
            );
          }),
          { numRuns: 20 },
        );
...
const planStateArb: fc.Arbitrary<PlanState> = fc.record({
  ...
  lastSlipUpAt: safeNullableIsoArb3,
});

const stepProgressArb3: fc.Arbitrary<Record<number, StepProgress>> =
  fc.dictionary(
    fc.nat({ max: 40 }).map(String),
    fc.record({
      step: fc.nat({ max: 40 }),
...
describe("Property 3: Hydration Completes Exactly Once", () => {
  ...
  it("HYDRATE is idempotent: dispatching it multiple times does not corrupt state", () => {
    fc.assert(
      fc.property(appStateArb, (payload) => {
        // Apply HYDRATE once
        const once = appReducer(INITIAL_APP_STATE, {
          type: "HYDRATE",
          payload,
        });
  ...
  it("HYDRATE followed by UPDATE_LAST_OPENED reflects the correct sequence (simulates hydration flow)", () => {
    fc.assert(
      fc.property(appStateArb, safeIsoArb3, (payload, nowIso) => {
        ...
        const afterHydrate = appReducer(INITIAL_APP_STATE, {
          type: "HYDRATE",
          payload,
        });
        ...
        expect(afterUpdate.cravingSessions).toEqual(
          afterHydrate.cravingSessions,
        );
...
describe("Property 5: Foreground Event Dispatches UPDATE_LAST_OPENED", () => {
  ...
  it("UPDATE_LAST_OPENED sets lastOpenedAt to the exact ISO string dispatched on foreground", () => {
    ...
    fc.assert(
      fc.property(appStateArb, foregroundIsoArb, (state, foregroundIso) => {
        const result = appReducer(state, {
          type: "UPDATE_LAST_OPENED",
  ...
  it("foreground after background: state saved on background is updated with new lastOpenedAt on foreground", () => {
    ...
    const foregroundIsoArb = fc
      .integer({
        min: new Date("2025-01-02").getTime(),
        max: new Date("2030-01-01").getTime(),
      })
      .map((ms) => new Date(ms).toISOString());
    ...
    fc.assert(
      fc.property(
        ...
        (state, backgroundTs, foregroundTs) => {
          ...
          expect(stateAfterForeground.triggerLogs).toEqual(
            stateAtBackground.triggerLogs,
          );
          expect(stateAfterForeground.planState).toEqual(
            stateAtBackground.planState,
          );
          expect(stateAfterForeground.userProfile).toEqual(
            stateAtBackground.userProfile,
          );
        },
      ),
      { numRuns: 100 },
    ...
    fc.assert(
      fc.property(
        ...
        (steps) => {
          ...
          const activeState = appReducer(INITIAL_APP_STATE, {
            type: "ACTIVATE_PLAN",
          });
    ...
    fc.assert(
      fc.property(
        ...
        (step, repeatCount) => {
          ...
          expect(
            completedSteps.filter((s) => s === step).length,
...
describe("Property 15: useProgressStats Computation Correctness", () => {
  ...
  it("smokeFreeDays equals Math.floor((Date.now() - lastSlipUpAt || activatedAt) / 86400000)", () => {
    fc.assert(
      fc.property(
        userProfileArb15,
        planStateWithActivatedAtArb15,
        (profile, planState) => {
          const before = Date.now();
          const result = computeProgressStats(profile, planState, []);
          const after = Date.now();

          const baseDateMs = new Date(
            planState.lastSlipUpAt || planState.activatedAt!,
          ).getTime();
          const diffMin = before - baseDateMs;
          const diffMax = after - baseDateMs;
  ...
  it("totalSavedCigarettes equals totalSmokeFreeDays * cigarettesPerDay", () => {
    fc.assert(
      fc.property(
        userProfileArb15,
        planStateWithActivatedAtArb15,
        (profile, planState) => {
          const result = computeProgressStats(profile, planState, []);
          expect(result.totalSavedCigarettes).toBe(
            result.totalSmokeFreeDays * profile.cigarettesPerDay,
          );
        },
      ),
      { numRuns: 100 },
    ...
    fc.assert(
      fc.property(
        ...
        (profile, planState) => {
          const result = computeProgressStats(profile, planState, []);
          const expected =
            (result.totalSavedCigarettes / profile.cigarettesPerPack) *
            profile.cigarettePricePerPack;
...
const milestonesRecordArb16: fc.Arbitrary<Record<number, string>> = fc
  .array(fc.tuple(fc.constantFrom(...MILESTONE_STEPS), safeIsoArb16), {
  ...
  it("fully populated milestones record yields no null achievedAt values", () => {
    fc.assert(
      fc.property(
        fc.record(
          Object.fromEntries(
            MILESTONE_STEPS.map((s) => [s, safeIsoArb16]),
    ...
    fc.assert(
      fc.property(
        ...
        (recentLogs) => {
          ...
          const actualCount = recentLogs.filter(
            (l) => l.type === result!.topTrigger,
          ).length;
...
describe("Property 13: Haptic Feedback on Key Actions", () => {
  ...
  it("step completion triggers notificationAsync(Success)", async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 1, max: 41 }), async (_step) => {
  ...
  it("checklist item toggle triggers impactAsync(Light)", async () => {
    await fc.assert(
      fc.asyncProperty(fc.uuid(), async (_itemId) => {
  ...
  it("haptic calls are wrapped in try-catch — errors do not propagate", async () => {
    ...
    await fc.assert(
      fc.asyncProperty(fc.boolean(), async (isStepComplete) => {
  ...
  it("ADD_CRAVING_SESSION preserves TriggerType in triggerId", () => {
    fc.assert(
      fc.property(triggerTypeArb1, (triggerType) => {
        ...
        const stored =
          result.cravingSessions[result.cravingSessions.length - 1];
        ...
        (triggerTypes) => {
          ...
          for (const triggerType of triggerTypes) {
            state = appReducer(state, {
              type: "ADD_CRAVING_SESSION",
              payload: {
                id: `cs_${require("crypto").randomUUID()}`,
                startTime: new Date().toISOString(),
                endTime: null,
                intensity: 5,
                outcome: null,
...
describe("Property 11: Search Filter Correctness", () => {
  ...
  it("empty query returns all items unchanged (dua filter)", () => {
    fc.assert(
      fc.property(fc.array(contentItemArb, { maxLength: 20 }), (items) => {
  ...
  it("empty query returns all items unchanged (library filter)", () => {
    fc.assert(
      fc.property(fc.array(libraryItemArb, { maxLength: 20 }), (items) => {
  ...
  it("items NOT matching the query are excluded from results", () => {
    ...
    fc.assert(
      fc.property(fc.string({ minLength: 3, maxLength: 8 }), (uniqueQuery) => {
        const matchingItem = {
          id: "match-1",
...
describe("Property 20: FlatList keyExtractor Returns Item ID", () => {
  ...
  it("keyExtractor returns the item id for any item with an id field", () => {
    fc.assert(
      fc.property(fc.record({ id: fc.uuid() }), (item) => {
      ...
      fc.property(
        ...
        (date) => {
          ...
          expect(fileName).toMatch(
            /^dhoa-mukt-path-backup-\d{4}-\d{2}-\d{2}\.json$/,
          );
        },
      ),
      { numRuns: 100 },
...
describe("Property 14: Milestone Share Message Completeness", () => {
  ...
  function buildShareMessage(
    milestone: { titleBangla: string; achievementBadge?: string },
    steps: number,
  ): string {
...
describe("Property 9: Theme Token Completeness", () => {
  ...
  it("all lightTheme color values are valid hex strings", () => {
    for (const key of REQUIRED_COLOR_KEYS) {
      const value = (lightTheme.colors as unknown as Record<string, string>)[
        key
      ];
  ...
  it("all darkTheme color values are valid hex strings", () => {
    for (const key of REQUIRED_COLOR_KEYS) {
      const value = (darkTheme.colors as unknown as Record<string, string>)[
        key
      ];
      expect(value).toMatch(HEX_COLOR_REGEX);
    }
  ...
  it("detectMilestone returns null when unique count does not match any milestone value", () => {
    fc.assert(
      fc.property(
        ...
        fc
          .integer({ min: 1, max: 41 })
          .filter((n) => !MILESTONE_VALUES.includes(n as any)),
        (nonMilestoneCount) => {
          // Build exactly nonMilestoneCount unique steps
          const completedSteps = Array.from(
            { length: nonMilestoneCount },
            (_, i) => i + 1,
          );
          const achievedMilestones: Record<number, string> = {};
...
function generateCravingSessionId(): string {
  return `cs_${require("crypto").randomUUID()}`;
}

function generateTriggerLogId(): string {
  return `tl_cr_${require("crypto").randomUUID()}`;
}
  ...
  it("object missing at least one required field returns false", () => {
    fc.assert(
      fc.property(
        ...
        fc.subarray([...REQUIRED_IMPORT_FIELDS], {
          minLength: 1,
          maxLength: REQUIRED_IMPORT_FIELDS.length,
        }),
        fc.record({
          userProfile: fc.anything(),
          planState: fc.anything(),
      ...
      fc.property(
        ...
        fc.dictionary(
          fc
            ...
            .filter((k) => !REQUIRED_IMPORT_FIELDS.includes(k as any)),
          fc.anything(),
          { maxKeys: 5 },
        ),
        ...
        (currentState, fieldsToOmit, baseImport) => {
          ...
          const stateAfter = isValid
            ? appReducer(currentState, {
                type: "HYDRATE",
                payload: incompleteImport as any,
              })
            : currentState;
