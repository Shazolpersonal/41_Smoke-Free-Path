    ...
    fc.assert(
      fc.property(
        ...
        (profile, [activatedAt, planState]) => {
          ...
          const expectedSavedCigarettes =
            stats.smokeFreeDays * profile.cigarettesPerDay;
          const expectedSavedMoney =
            (expectedSavedCigarettes / profile.cigarettesPerPack) *
            originalPrice;
        ...
        (step, [activatedAt, planState]) => {
          ...
          const initialState: AppState = {
            ...
            planState: {
              ...
              isActive: true,
              activatedAt,
              completedSteps: planState.completedSteps.filter(
                (s) => s !== step,
              ),
            },
          };
    ...
    fc.assert(
      fc.property(
        ...
        (profile, [activatedAt, planState]) => {
          ...
          const diffBefore = Math.floor(
            (before - new Date(activatedAt).getTime()) / MS_PER_DAY,
          );
          const diffAfter = Math.floor(
            (after - new Date(activatedAt).getTime()) / MS_PER_DAY,
          );
          ...
          expect(stats.totalSavedCigarettes).toBe(
            stats.smokeFreeDays * profile.cigarettesPerDay,
          );
        },
      ),
...
describe("2.4 Code Quality Preservation Tests", () => {
  test("Preservation: TRIGGER_LABELS, TOTAL_STEPS work from constants/index.ts", () => {
    ...
    for (const type of expectedTriggerTypes) {
      ...
      expect(typeof TRIGGER_LABELS[type as keyof typeof TRIGGER_LABELS]).toBe(
        "string",
      );
    }
        ...
        (legacyState) => {
          ...
          if (legacyState.userProfile.quitDate) {
            const expectedActivatedAt = new Date(
              legacyState.userProfile.quitDate,
            ).toISOString();
            expect(migratedState.planState.activatedAt).toBe(
              expectedActivatedAt,
            );
          }
  ...
  fc.assert(
    fc.property(
      ...
      (profile, [activatedAt, planState]) => {
        ...
        expect(
          Math.abs(stats.smokeFreeDays - expectedSmokeFreeDays),
