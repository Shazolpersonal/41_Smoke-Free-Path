...
const activePlanStateArb = (activatedAt: string) =>
  fc.record<PlanState>({
    ...
    lastSlipUpAt: fc.option(safeISODateArb, { nil: null }),
  });
  ...
  fc.assert(
    fc.property(userProfileArb, pastISODateArb, (profile, activatedAt) => {
      ...
      const diffBefore = Math.floor(
        (before - new Date(activatedAt).getTime()) / 86_400_000,
      );
      const diffAfter = Math.floor(
        (after - new Date(activatedAt).getTime()) / 86_400_000,
      );
      ...
      const expectedMoney =
        (stats.totalSavedCigarettes / profile.cigarettesPerPack) *
        ...
        if (!planState.isActive) {
          ...
          if (planState.activatedAt) {
            ...
            const diffDays = Math.floor(
              (nowDateOnly - actDateOnly) / 86_400_000,
            );
          ...
          if (step > maxStepByActivation) {
            ...
            const d = planState.lastCompletedAt
              ? new Date(planState.lastCompletedAt)
              : null;
