...
export default function WeeklyTriggerChart() {
  ...
  const weeklyChartData = useMemo(() => {
    ...
    const recent = triggerLogs.filter(
    ...
    return (Object.entries(counts) as [TriggerType, number][]).map(
      ([type, count]) => ({
  ...
  return (
    <Card style={styles.chartCard}>
      {weeklyChartData.length === 0 ? (
        ...
        >
          ...
          <Typography
            variant="subheading"
            color="text"
            style={styles.boldTextCenter}
          >
            এই সপ্তাহে কোনো ট্রিগার লগ নেই
          </Typography>
          <Typography
            variant="body"
            color="textSecondary"
              ...
              >
                <View
                  style={[
                    styles.chartBarFill,
                    {
                      width: `${Math.round(percent * 100)}%` as DimensionValue,
                      backgroundColor: theme.colors.primary,
                    },
                  ]}
                />
      ...
      {weeklySummary && (
        <View
          style={[styles.chartSummary, { borderTopColor: theme.colors.border }]}
        >
