...
import { appReducer, INITIAL_APP_STATE, INITIAL_PLAN_STATE } from '@/context/AppContext';
...
const planStateArb = fc.record<PlanState>({
  ...
  completedSteps: fc.uniqueArray(fc.integer({ min: 1, max: 41 }), { maxLength: 41 }),
  lastCompletedAt: fc.option(isoDateArb, { nil: null }),
  totalResets: fc.integer({ min: 0, max: 20 }),
});

const inactivePlanStateArb = planStateArb.map((p) => ({ ...p, isActive: false }));

const triggerLogArb = fc.record<TriggerLog>({
  id: fc.uuid(),
  type: fc.constantFrom(
    'stress', 'boredom', 'social', 'environmental', 'habitual'
...
const cravingSessionArb: fc.Arbitrary<CravingSession> = fc.record({
  ...
  outcome: fc.option(
    fc.constantFrom('overcome', 'slipped', 'abandoned') as fc.Arbitrary<CravingSession['outcome'] & string>,
    { nil: null }
  ),
  strategiesUsed: fc.array(
    fc.constantFrom('deep_breathing', 'dua', 'walk', 'water', 'delay') as fc.Arbitrary<CravingSession['strategiesUsed'][number]>,
    { maxLength: 5 }
  ),
  triggerId: fc.option(
    fc.constantFrom('stress', 'boredom', 'social', 'environmental', 'habitual') as fc.Arbitrary<CravingSession['triggerId'] & string>,
    { nil: null }
  ),
...
test('Property 2: ACTIVATE_PLAN Action State Mutation', () => {
  fc.assert(
    fc.property(
      appStateArb.map((s) => ({ ...s, planState: { ...s.planState, isActive: false } })),
      (state) => {
        ...
        expect(next.planState.completedSteps).toEqual(state.planState.completedSteps);
...
test('Property 4: TOGGLE_CHECKLIST_ITEM Two-Way Toggle', () => {
  fc.assert(
    fc.property(
      ...
      (state, step, itemId) => {
        const action = { type: 'TOGGLE_CHECKLIST_ITEM' as const, payload: { step, itemId } };
        ...
        appStateArb
          .map((s) => ({
            ...
            planState: {
              ...s.planState,
              isActive: true,
              completedSteps: s.planState.completedSteps.filter((cs) => cs !== step),
            },
          }))
          .map((s) => ({ state: s, step }))
      ),
      ({ state, step }) => {
        const next = appReducer(state, { type: 'COMPLETE_STEP', payload: step });
        ...
        expect(next.planState.currentStep).toBe(Math.max(state.planState.currentStep, Math.min(step + 1, 41)));
