...
function makeState(lastStreakDate: string | null, dailyStreak: number): AppState {
...
test('Property 1: Daily Streak Idempotence', () => {
  fc.assert(
    fc.property(isoDateStrArb, streakArb, (today, streak) => {
      ...
      const after1 = appReducer(state, { type: 'UPDATE_LAST_OPENED', payload: isoDatetime });
      const after2 = appReducer(after1, { type: 'UPDATE_LAST_OPENED', payload: isoDatetime });
      const after3 = appReducer(after2, { type: 'UPDATE_LAST_OPENED', payload: isoDatetime });
